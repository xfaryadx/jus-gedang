...
class BananaBot {
    ...
    async sendTelegramNotification(bananaName, peelLimit, price) {
        const fetch = (await import('node-fetch')).default; // Add this line
        const botToken = 'BOT_TOKEN';
        const chatId = 'ID_TELE';
        const message = `${bananaName}| Price: ${price}`;
        ...
        try {
            ...
            this.log('Error sending Telegram notification: ' + error.message);
        }
    }
    ...
    async processAccount(queryId, isFirstAccount = false, doQuests) {
        ...
        if (token) {
            ...
            try {
                ...
                // New code to count bananas
                const bananaListResponse = await axios.get(`${this.base_url}/get_banana_list`, { headers: this.headers });
                const bananas = bananaListResponse.data.data.banana_list;
                const collectedBananas = bananas.filter(banana => banana.count > 0).length;
                this.log(colors.green(`Collected : ${colors.white(collectedBananas)} Banana`));
                    ...
                    if (remainLotteryCount > 0) {
                        ...
                        for (let i = 0; i < remainLotteryCount; i++) {
                            ...
                            if (doLotteryResponse.status === 200) {
                                ...
                                if (sellExchangeUsdt !== 'N/A') {
                                    await this.sendTelegramNotification(bananaName, lotteryResult.daily_peel_limit || 'N/A', `${sellExchangePeel} Peel, ${sellExchangeUsdt} USDT`);
                                }
                            } else {
    ...
    for (let i = Math.floor(seconds); i >= 0; i--) {
        ...
        process.stdout.write(`===== Completed all accounts, waiting ${hours} hours ${minutes} minutes ${secs} seconds to continue the loop =====`);
    ...
    const cooldownAnswer = await this.askQuestion('Enter cooldown time after all complete (in minutes): ');
    const cooldownMinutes = parseInt(cooldownAnswer, 10) || 10; // Default to 10 minutes if input is invalid
    ...
    while (true) {
        ...
        for (let i = 0; i < userData.length; i++) {
            ...
            await this.Countdown(cooldownMinutes * 60 ); // Use the user-defined cooldown time
